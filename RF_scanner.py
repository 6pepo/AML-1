...
acc_list, acc_err_list, sens_list, sens_err_list, spec_list, spec_err_list = RF.RF_binary_scanner(tree_range, k_range, tot_pattern, tot_labels, label0, label1)

fig, ax = plt.subplots(3, 1, sharex=True)
fig.subplots_adjust(0.2, 0.2)

ax[0].scatter(tree_range, acc_list[:, 0], color='C0')
ax[0].errorbar(tree_range, acc_list[:, 0], yerr=acc_err_list[:, 0], color='C0')
ax[0].grid(True)
ax[0].set_ylim(0, 1.2)
ax[0].set_title('Accuracy')

ax[1].scatter(tree_range, sens_list[:, 0], color='C1')
ax[1].errorbar(tree_range, sens_list[:, 0], yerr=sens_err_list[:, 0], color='C1')
ax[1].grid(True)
ax[1].set_ylim(0, 1.2)
ax[1].set_title('Sensitivity')

ax[2].scatter(tree_range, spec_list[:, 0], color='C2')
ax[2].errorbar(tree_range, spec_list[:, 0], yerr=spec_err_list[:, 0], color='C2')
ax[2].grid(True)
ax[2].set_ylim(0, 1.2)
...
def update(val):
    ...
    ax[0].clear()
    ax[0].scatter(tree_range, acc_list[:, i], color='C0')
    ax[0].errorbar(tree_range, acc_list[:, i], yerr=acc_err_list[:, i], color='C0')
    ax[0].grid(True)
    ax[0].set_ylim(0, 1.2)
    ax[0].set_title('Accuracy')

    ax[1].clear()
    ax[1].scatter(tree_range, sens_list[:, i], color='C1')
    ax[1].errorbar(tree_range, sens_list[:, i], yerr=sens_err_list[:, i], color='C1')
    ax[1].grid(True)
    ax[1].set_ylim(0, 1.2)
    ax[1].set_title('Sensitivity')

    ax[2].clear()
    ax[2].scatter(tree_range, spec_list[:, i], color='C2')
    ax[2].errorbar(tree_range, spec_list[:, i], yerr=spec_err_list[:, i], color='C2')
    ax[2].grid(True)
    ax[2].set_ylim(0, 1.2)
    ax[2].set_title('Specificity')
    ax[2].set_xlabel('Number of Trees')
...
scores = np.concatenate((acc_list, sens_list, spec_list), axis = 0)
normalization = colors.Normalize(vmin=np.min(scores), vmax=np.max(scores))

ax_acc = fig3d.add_subplot(131)
ax_acc.pcolormesh(k_range, tree_range, acc_list, norm=normalization, cmap=cm.plasma)
ax_acc.set_ylabel("Number of trees")
ax_acc.set_xlabel("Number of folds")
ax_acc.set_title("Accuracy")

ax_sens = fig3d.add_subplot(132)
ax_sens.pcolormesh(k_range, tree_range, sens_list, norm=normalization,cmap=cm.plasma)
...
colormesh = ax_spec.pcolormesh(k_range, tree_range, spec_list, norm=normalization, cmap=cm.plasma)
